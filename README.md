...
This fork contains several changes to improve UI reporting and triage efficiency:
- **Interactive Local UI with Approve/Acknowledge (New `serve` command)**: run `backstop serve` to host the report with a functional triage UI that automatically saves state to disk and seamlessly shifts to the next test result.
- **Auto-Cropping Screenshots**: Slices away excessive background color from the bottom of references and automatically bounds the test shots to match the height, all configured by `trimBottomColor` and `trimBottomPadding` in your scenario settings.
- **Enhanced Comparison UI**: Sticky headers so you can triage without losing your place, view filtering by viewport label, number indicators for previews, adjustable max-width viewing dimensions, and a sorting toggle for results.
- **Less Noisy Output**: Replace console.log messages with a progress bar
- **`regenerateReport` Command**: Programmatically rebuild your HTML reports without having to rerun the entire engine tests from scratch.
...
- In-browser reporting UI with...
  ...
  * configurable image max width
  * sorting toggles and viewport label filtering
  * scenario display filtering
  * reference, test, visual diff inspector
  * cool scrubber thingy
  * sticky comparison header
  * approving and acknowledging tests seamlessly through local API
...
```sh
backstop regenerateReport
```

This command will easily regenerate your Backstop HTML report based on the current raw output files (`report.json`), without having to rerun any of your test screenshots. This is  useful if you want to quickly rebuild it after a manual modification!
...
| `trimBottomColor`        | Hex color string (e.g. `"#FFFFFF"`) to trim from the bottom of Reference screenshots which is then enforced on Test captures. |
| `trimBottomPadding`      | Number of pixels to add as padding after performing a `trimBottomColor` operation. Default `0`                                |
...
Backstop web reports support *approving/acknowledging scenarios* and *viewing browser logs* generated by each scenario. To enable these features seamlessly, this fork provides a `serve` command that spins up a local interactive server that hosts your reports and runs the API backend for you:

```bash
backstop serve
```
The report server binds automatically to an open port (default `3000`), serving the `html_report` alongside your reference/test images so everything loads correctly. Pressing `Approve` or `Acknowledge` in the browser UI immediately contacts the backend and permanently triage-saves your state to disk while smartly sliding you down to your next test result!

Alternatively, if you only want to run the raw backend process in another terminal window without the static file host...
